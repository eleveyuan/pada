...
import pada.features.feature
import pada.features.transformer
...
class FeatureEngineeringPipeline(DataFrameMapper):
    ...
    def __init__(self, features: OneOrMore['pada.features.feature.BaseFeature']):
        if not features:
            _features = [
                pada.features.feature.BaseFeature(input=[],
    ...
    @property
    def pada_features(self) -> List['pada.features.feature.BaseFeature']:
...
class EngineerFeaturesResult(NamedTuple):
    X_df: pd.DataFrame
    features: List['pada.features.feature.BaseFeature']
